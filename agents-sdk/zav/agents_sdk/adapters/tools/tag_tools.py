...
class TagTools(ToolsSource):
    ...
    def __init__(
        ...
        tag_tools_config: Optional[TagToolsConfig] = None,
    ):
        self.__tags_service = tags_service
        self.__document_tools = document_tools
        self.__config = tag_tools_config or TagToolsConfig()
...
class TagToolsFactory(AgentDependencyFactory):
    ...
    def create(
        ...
        if not tags_service:
            raise ValueError("TagTools requires a TagsService instance")
        if not document_tools:
            raise ValueError("TagTools requires a DocumentTools instance")
        return TagTools(
