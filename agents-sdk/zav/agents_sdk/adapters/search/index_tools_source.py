...
class IndexToolsConfig(BaseModel):
    ...
    include_citation_instructions: bool = Field(
        False,
        description=(
            "Include citation format instructions in the tools prompt. "
            "Enable when SearchSkillsSource is excluded so the model "
            "still learns how to format citations."
        ),
    )
...
class IndexToolsSource(ToolsSource):
    ...
    def __init__(
        ...
        index_tools_config: IndexToolsConfig,
        citation_config: CitationConfig,
        citation_store: CitationStore,
    ):
        self.__retriever = retriever
        self.__config = index_tools_config
    ...
    def to_prompt(self) -> str:
        ...
        if self.__config.include_citation_instructions:
            parts.append(
                "### Citations\n" + build_citation_instructions(self.__citation_config)
            )
        if not parts:
            return ""
        return "\n\n".join(parts)
    ...
    @streamable(
        ...
        completed_text=(
            "Searched for: {{ query }}."
            " Read {{ results|length }}"
            " chunk{{ 's' if results|length != 1 else '' }}."
        ),
    ...
    async def __execute_search(
        ...
        effective_unit = self.__config.default_retrieval_unit
    ...
    @streamable(
        running_text=(
            "Browsing documents" "{{ ' matching: ' + query if query else '' }}..."
        ),
        completed_text=(
...
class IndexToolsSourceFactory(AgentDependencyFactory):
    ...
    def create(
        ...
        citation_store: CitationStore,
        index_tools_configuration: IndexToolsConfig = IndexToolsConfig(),
        citation_configuration: CitationConfig = CitationConfig(),
    ) -> IndexToolsSource:
        return IndexToolsSource(
            retriever=zav_retriever,
            index_tools_config=index_tools_configuration,
