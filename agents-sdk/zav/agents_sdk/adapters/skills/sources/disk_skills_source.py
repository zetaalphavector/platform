...
class DiskSkillsSourceConfiguration(BaseModel):
    enabled: bool = False
...
class DiskSkillsSource(SkillsSource):
    ...
    async def create_skill(self, name: str, content: str) -> SkillProperties:
        if not self.__directories:
            raise SkillWriteError("No skills directories configured.")

        properties = self.parse_skill_properties(content)

        target_dir = Path(self.__directories[0]) / name
        if target_dir.exists():
            raise SkillWriteError(f"Skill directory already exists: {target_dir}")
        ...
        try:
            ...
            try:
                ...
                pass
            raise SkillWriteError(f"Failed to write {skill_file}: {e}") from e

        properties.location = str(skill_file)
        self.__skills[properties.name] = properties
        self.__skill_bodies[properties.name] = self.parse_skill_body(content)
        self.__skill_paths[properties.name] = target_dir
        self.__skill_resources[properties.name] = []

        logger.info(f"Created skill: {properties.name} at {target_dir}")
        return properties
...
class DiskSkillsSourceFactory(AgentDependencyFactory):
    ...
    def create(
        cls,
        disk_skills_source_configuration: DiskSkillsSourceConfiguration = (
            DiskSkillsSourceConfiguration()
        ),
    ) -> DiskSkillsSource:
        return DiskSkillsSource(
            directories=disk_skills_source_configuration.skills_directories,
            enabled=disk_skills_source_configuration.enabled,
        )
