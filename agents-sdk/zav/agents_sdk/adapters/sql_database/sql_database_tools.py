...
class SQLDatabaseToolsConfig(BaseModel):
...
class SQLDatabaseTools:
    def __init__(self, config: SQLDatabaseToolsConfig):
        self._config = config
    ...
    def _validate_result_name(self, name: str) -> None:
        """Result name should start with a letter or underscore and contain only
    ...
    def _is_table_allowed(self, schema: Optional[str], table: str) -> bool:
        """Check if access to a table is allowed based on configuration."""
        if self._config.allowed_schemas is not None:
            if schema and schema not in self._config.allowed_schemas:
                return False

        if self._config.allowed_tables is not None:
            full_name = f"{schema}.{table}" if schema else table
            if full_name not in self._config.allowed_tables and (
    ...
    def _validate_table_access(self, schema: Optional[str], table: str) -> None:
        if not self._is_table_allowed(schema, table):
            full_name = f"{schema}.{table}" if schema else table
    ...
    def _get_table(self, table_name: str) -> Table:
        ...
        self._validate_table_access(schema, table)

        table_obj = Table(
            table, self._metadata, schema=schema, autoload_with=self._engine
        )
        self._cached_tables[table_name] = table_obj
        return table_obj

    def _resolve_stored_value(self, filter_condition: SQLFilterCondition) -> Any:
        if filter_condition.stored_value_reference not in self._stored_values:
            raise ValueError(
                f"Stored value '{filter_condition.stored_value_reference}' "
                "not found."
            )
        return self._stored_values[filter_condition.stored_value_reference]

    def _build_filter_clause(
        self, source: Union[Table, Select, Subquery], filters: List[SQLFilterCondition]
    ) -> Any:
        conditions = []
        for f in filters:
            col = source.c[f.column]  # type: ignore[union-attr]
            value = (
                self._resolve_stored_value(f) if f.stored_value_reference else f.value
            )
    ...
    def _plan_to_cte(self, plan: QueryPlan, cte_refs: Dict[str, Select]) -> Select:
        source_name = plan.sources[0]
        if source_name not in self._query_plans:
            source_table: Union[Table, Select] = self._get_table(source_name)
        else:
        ...
        if plan.operation == OperationType.SELECT:
            ...
            if isinstance(source_table, Table):
                where_clause = self._build_filter_clause(source_table, filters)
                return select(source_table).where(where_clause)
            else:
                subq = source_table.subquery()
                where_clause = self._build_filter_clause(subq, filters)
            ...
            agg_expr = self._build_aggregation_expr(
                col, aggregate_params.operation, aggregate_params.column
            )
            ...
            if right_name not in self._query_plans:
                right_table: Union[Table, Select] = self._get_table(right_name)
            else:
            ...
            return self._build_join(source_table, right_table, on_cols, join_params.how)

        raise ValueError(f"Unknown operation type: {plan.operation}")

    def _execute_query_plan(
        self, target: str, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        if target not in self._query_plans:
        ...
        plans = list(self._query_plans.keys())
        cte_refs: Dict[str, Select] = {}

        for name in plans:
            plan = self._query_plans[name]
            query = self._plan_to_cte(plan, cte_refs)
        ...
        try:
            ...
            for s in schemas_to_check:
                for table_name in inspector.get_table_names(schema=s):
                    if self._is_table_allowed(s, table_name):
                        tables.append({"schema": s, "table": table_name})

            return {"status": "ok", "tables": tables}

        except Exception as e:
            self.debug(f"Error listing tables: {e}")
    ...
    def get_table_schema(self, table_name: str) -> Dict[str, Any]:
        ...
        Returns:
            dict: JSON with keys:
                ...
                - columns (list): List of column info dicts with name, type,
                  nullable, primary_key, and sample_values.
        ...
        try:
            ...
            self.debug(f"Error getting table schema: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def preview_data(self, input_name: str, n_rows: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            rows = self._execute_query_plan(input_name, limit=n_rows)
            return {
                "status": "ok",
            ...
            self.debug(f"Error previewing data: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def read_data(
        ...
        """
        try:
            max_rows = min(to_row - from_row, self._config.max_rows_per_query, 500)
            if max_rows <= 0:
                return {
                    "status": "error",
            ...
            if input_name not in self._query_plans:
                table = self._get_table(input_name)
                ...
                all_rows = self._execute_query_plan(input_name, limit=to_row)
            ...
            self.debug(f"Error reading data: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def count_rows(
        ...
        """Count rows in a table or stored result, optionally with filters.
        ...
        try:
            if input_name not in self._query_plans:
                ...
                if filters:
                    where_clause = self._build_filter_clause(table, filters)
                    query = query.where(where_clause)

                with self._engine.connect() as conn:
                    row_count = conn.execute(query).scalar()
            else:
                rows = self._execute_query_plan(input_name)
                row_count = len(rows)

            return {"status": "ok", "row_count": row_count}

        except Exception as e:
            self.debug(f"Error counting rows: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def get_row_by_pk(self, table_name: str, pk: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._engine.connect() as conn:
                ...
                row = result.fetchone()
                if row:
                    return {"status": "ok", "row": dict(row._mapping)}
                else:
                    return {"status": "ok", "row": None}

        except Exception as e:
            self.debug(f"Error getting row by PK: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def select_columns(
        ...
        """
        try:
            self._validate_result_name(output_result_name)

            self._query_plans[output_result_name] = QueryPlan(
                sources=[input_name],
            ...
            self.debug(f"Error selecting columns: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def get_column_values(
        ...
        """
        try:
            self._validate_result_name(output_variable_name)
            ...
            self._stored_values[output_variable_name] = values
            ...
            self.debug(f"Error getting column values: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def filter_table_rows(
        ...
        try:
            ...
            self.debug(f"Error filtering rows: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def sort_table_rows(
        ...
        try:
            ...
            self.debug(f"Error sorting rows: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def aggregate_column(
        ...
        Args:
            ...
            operation: Aggregation operation ('sum', 'mean', 'min', 'max', 'count').
            output_result_name: Name to store the aggregated result under.
        ...
        try:
            ...
            try:
                ...
                return {
                    "status": "error",
                    "message": f"Invalid operation: '{operation}'. Must be one of "
            ...
            self.debug(f"Error aggregating column: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def join_tables(
        ...
        try:
            ...
            try:
                ...
                return {
                    "status": "error",
                    "message": f"Invalid join type: '{how}'. Must be one of "
            ...
            self.debug(f"Error joining tables: {e}")
            return {"status": "error", "message": str(e)}
    ...
    def insert_row(self, table_name: str, row: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            with self._engine.connect() as conn:
                ...
                conn.commit()
            return {"status": "ok", "table_name": table_name, "row": row}
        except Exception as e:
            self.debug(f"Error inserting row: {e}")
            return {"status": "error", "message": str(e)}
...
class SQLDatabaseToolsFactory(AgentDependencyFactory):
    @classmethod
    def create(
        cls,
        sql_database_tools_config: Optional[SQLDatabaseToolsConfig] = None,
    ) -> Optional[SQLDatabaseTools]:
        if sql_database_tools_config is None:
            return None
        return SQLDatabaseTools(config=sql_database_tools_config)
