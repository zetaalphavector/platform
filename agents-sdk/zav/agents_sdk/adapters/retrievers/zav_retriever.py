...
from zav.search_api.apis import (
    DocumentAssetsApi,
    DocumentsApi,
    SourcesApi,
    TenantSettingsApi,
...
class ZAVRetriever:
    def __init__(
        ...
        self.__tenant_settings = TenantSettingsApi(api_client)
        self.__sources = SourcesApi(api_client)
    ...
    @handle_api_errors
    async def get_tenant_settings(self) -> Dict:
        response = await asyncify(self.__tenant_settings.retrieve_tenant_settings)(
            tenant=self.__tenant,
            **(
                {"requester_uuid": self.__internal_headers["requester_uuid"]}
                if "requester_uuid" in self.__internal_headers
                else {}
            ),
            **(
                {"user_tenants": self.__internal_headers["user_tenants"]}
                if "user_tenants" in self.__internal_headers
                else {}
            ),
        )
    ...
    async def get_sources(self) -> List[Dict]:
        response = await asyncify(self.__sources.source_list)(
            tenant=self.__tenant,
            **(
                {"user_roles": self.__internal_headers["user_roles"]}
                if "user_roles" in self.__internal_headers
                else {}
            ),
            **(
                {"user_tenants": self.__internal_headers["user_tenants"]}
        ...
        if hasattr(response, "value"):
            return [item.to_dict() for item in response.value]
